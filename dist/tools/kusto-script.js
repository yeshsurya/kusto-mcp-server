...
export async function handleKustoScript(params) {
    const { scriptPath, connectionName, database, multiLine, quitOnError, timeout, format = 'json', } = params;
    ...
    if (!existsSync(scriptPath)) {
        throw new KustoMcpError(`Script file not found: ${scriptPath}`, ErrorCodes.SCRIPT_NOT_FOUND, { scriptPath });
    }
    // Get connection
    let connection;
    if (connectionName) {
        ...
        if (!connection) {
            throw createValidationError('No connection specified and no default connection configured.', 'connectionName');
        }
    }
...
//# sourceMappingURL=kusto-script.js.map