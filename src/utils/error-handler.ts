...
export function createConnectionError(connectionName: string): KustoMcpError {
  return new KustoMcpError(
    `Connection '${connectionName}' not found. Use kusto_connection_list to see available connections or kusto_connection_add to create one.`,
    ErrorCodes.CONNECTION_NOT_FOUND,
    { connectionName }
  );
}

export function createQueryError(message: string, details?: unknown): KustoMcpError {
  return new KustoMcpError(
    message,
...
export function createValidationError(message: string, parameter?: string): KustoMcpError {
  return new KustoMcpError(
    message,
...
export function getAuthErrorHelp(errorMessage: string): string {
