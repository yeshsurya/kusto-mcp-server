...
export function parseCSVOutput(output: string): ParsedOutput {
...
export function formatAsTable(parsed: ParsedOutput): string {
  if (parsed.columns.length === 0) {
    return 'No results';
  }

  // Calculate column widths
  const widths: number[] = parsed.columns.map(col => col.length);
...
export function parseSchemaOutput(output: string): { tables: Array<{ name: string; columns: Array<{ name: string; type: string }> }> } {
  const parsed = parseCSVOutput(output);
  const tables: Map<string, Array<{ name: string; type: string }>> = new Map();
