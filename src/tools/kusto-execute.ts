...
import { getConnection, getDefaultConnection, type AuthMethod } from '../config/connections.js';
...
export const kustoExecuteTool: Tool = {
  name: 'kusto_execute',
...
export async function handleKustoExecute(params: KustoExecuteParams): Promise<string> {
  const {
    commands,
    connectionName,
    connectionString,
    ...
    if (connectionName) {
      ...
      if (!connection) {
        throw createValidationError(
          'No connection specified. Provide connectionName, connectionString, or configure a default connection.',
          'connectionName'
        );
      }
    }
