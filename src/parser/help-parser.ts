import type { CliOption, OptionCategory, ParsedHelp } from './option-types.js';
...
function categorizeOption(name: string, description: string): OptionCategory {
...
function parseValueType(valueName: string): 'string' | 'number' | 'boolean' | 'path' {
...
export function parseHelpOutput(helpText: string): ParsedHelp {
  ...
  let currentOption: Partial<CliOption> | null = null;
  ...
  for (const line of lines) {
    ...
    if (trimmed.toLowerCase() === 'examples:') {
      ...
      if (currentOption?.name) {
        options.push(currentOption as CliOption);
        currentOption = null;
      }
      continue;
    }
...
export function getKnownOptions(): CliOption[] {
